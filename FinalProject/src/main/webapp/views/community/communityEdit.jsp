  ...
  <div class="form-box">
    ...
    <form action="/community/update" method="post" enctype="multipart/form-data">
      ...
      <div class="form-group">
        ...
        <input type="text" id="commuSubject" name="commuSubject" value="${commu.commuSubject}" required placeholder="제목을 입력하세요" />
      </div>

      <div class="form-group">
        <label for="commuContents">내용</label>
        <textarea id="commuContents" name="commuContents" required placeholder="내용을 입력하세요">${commu.commuContents}</textarea>
      </div>
      ...
      <div class="form-group">
        ...
        <c:if test="${not empty commu.commuImageStoreName}">
          <div class="existing-image">
            <img src="/upload/community/${commu.commuImageStoreName}" alt="기존 이미지" />
          </div>
        </c:if>
        <c:if test="${empty commu.commuImageStoreName}">
          <p style="color: #999; font-style: italic; margin-top: 10px;">등록된 이미지가 없습니다.</p>
        </c:if>
      </div>
